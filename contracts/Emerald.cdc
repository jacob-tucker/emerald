...
// Token contract for Emerald token
pub contract Emerald: FungibleToken {

    // Total supply of Emerald tokens in existence
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @Emerald.Vault
        ...
        destroy() {
            Emerald.totalSupply = Emerald.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        //
        pub fun mintTokens(amount: UFix64): @Emerald.Vault {
            pre {
                amount > (0.0 as UFix64): "Amount minted must be greater than zero"
                amount <= self.allowedAmount: "Amount minted must be less than the allowed amount"
            }
            Emerald.totalSupply = Emerald.totalSupply + amount
    ...
    init() {
        ...
        self.TokenPublicBalancePath = /public/EmeraldBalance
        self.TokenMinterStoragePath = /storage/EmeraldMinter
        ...
        self.account.link<&Emerald.Vault{FungibleToken.Receiver}>(
        ...
        self.account.link<&Emerald.Vault{FungibleToken.Balance}>(
            self.TokenPublicBalancePath,
            target: self.TokenStoragePath
        )

        let admin <- create Administrator()
        self.account.save(<-admin, to: /storage/EmeraldAdmin)
